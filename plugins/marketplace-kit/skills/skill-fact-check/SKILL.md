...
This skill's source lives only in `custom-agent-skills` (it ships in `marketplace-kit`). A routine clones **all** its attached repos into one session — typically `custom-agent-skills` **and** `private-custom-agent-skills`. So:
...
```json
{
  "unit_id": "apple-app-store-screenshots/apple-app-store-screenshots",
  "plugin_dir": "plugins/apple-app-store-screenshots",
  "claims": [
    {
