...
1. Run `preflight`. Stop the whole project only for malformed project-level configuration or an unreconciled owned delivery artifact. Isolate target, retrieval, research, edit, and validation failures per skill.
2. Select skills from the request. Use `due-set` for `run due`; interactive `run` presents choices and supports `all`. A no-question run never asks for choices.
3. Acquire one project-level run lock. Recheck the starting manifest and delivery revision immediately before writing; stop rather than merging competing state.
