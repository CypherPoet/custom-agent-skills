...
2. Acquire one project-level run lock. For `githubPullRequest` delivery, establish the marked stable branch as the working state before selecting skills: fetch the default and owned branches, validate ownership, create or reuse an isolated worktree for the owned branch, and incorporate the latest default branch without rewriting history. Perform every remaining helper operation against that working state. For `localReport`, continue in the resolved project root.
3. Run the authoritative `preflight` against the working state, then select skills from the request. Use `due-set` there for `run due`; interactive `run` presents choices and supports `all`. A no-question run never asks for choices. Recheck the starting manifest and delivery revision immediately before writing; stop rather than merging competing state.
...
6. Ask only: “Given these configured sources, what should change in this skill?” Do not enumerate every factual statement. Identify supported corrections, material improvement suggestions, human decisions, and retrieval or processing failures. Produce the structured result defined in `assets/research-result.schema.v1.json`.
7. Validate the structured result before it can affect files or state. Treat malformed or internally inconsistent output as an incomplete attempt.
