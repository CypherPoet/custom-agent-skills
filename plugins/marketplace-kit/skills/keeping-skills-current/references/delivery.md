...
2. Compare any local owned tip with the fetched tip. Use the newer owned tip when one is a linear descendant; stop on divergence or uncertain ownership. When no owned tip exists, use the fetched default tip as the preview base. Record a local-ahead relationship as pending delivery or reconciliation; committed local state is not eligible for the no-due exit merely because its fingerprints are fresh.
...
5. If nothing is due, no relevant owned-worktree edits require reconciliation, and the local owned tip is not ahead, delete the disposable worktree and make no owned-branch, manifest, report, commit, push, or pull-request change. If the local owned tip is ahead, do not report `No skills are due.` or strand its commits: stop with explicit pending-delivery reconciliation guidance. An interactive run may verify and finish a durable workflow-owned pending delivery; never infer or reconstruct missing report content.
