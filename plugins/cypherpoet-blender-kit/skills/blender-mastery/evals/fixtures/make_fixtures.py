...
def make_bracket_dirty(out_dir):
    ...
    # Two abutting boxes forming an L; created separately so the seam verts
    # are exact duplicates (the "forgot to merge" defect).
    ...
    assert any(abs(s - 1.0) > 1e-3 for s in obj.scale), "fixture bug: scale applied"
...
def make_part(out_dir):
    ...
    inset = bmesh.ops.inset_individual(bm, faces=[top], thickness=0.5)
    bm.faces.ensure_lookup_table()
    plateau = max(bm.faces, key=lambda f: f.calc_center_median().z)
