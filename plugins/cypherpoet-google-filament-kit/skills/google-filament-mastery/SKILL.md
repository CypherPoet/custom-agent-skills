...
- **Using a binding/wrapper? The engine's full API is *not* the binding's API.** When you go through react-native-filament, filament.js, or a custom JSI layer rather than the C++ engine directly, the wrapper bridges only a **subset** of the engine. A feature existing in the bundled engine headers does **not** mean it's reachable from your language — verify against the *binding's* registered surface, not the engine docs. Concrete, audited example (**react-native-filament 1.11.0**, which vendors the full v1.72.0 engine headers): the headers contain `setScreenSpaceReflectionsOptions`, `setColorGrading`, `setShadowType`, `Camera::setExposure`, and `setFrontFaceWindingInverted`, but its JSI layer does **not** bridge any of them — so SSR, color grading, soft-shadow *type*, camera exposure, and winding-invert are **unreachable from JS**. What *is* exposed: SSAO, bloom, TAA, FXAA, dithering, per-entity `setCastShadow`/`setReceiveShadow`, shadowing on/off, and IBL. Check the binding's own registered methods — for RN-Filament that's `cpp/core/RNF*Wrapper.cpp` (the `registerHybridMethod("…")` calls) cross-checked with `src/types/*.ts` — before promising a feature.
