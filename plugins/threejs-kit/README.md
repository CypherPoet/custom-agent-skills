# threejs-kit

Three.js / WebGPU / WebGL tooling.

## Installation

Install via the [`cypherpoet-toolchest`](https://github.com/CypherPoet/cypherpoet-toolchest) marketplace:

```shell
# Skip if you've already added this marketplace
/plugin marketplace add CypherPoet/cypherpoet-toolchest

# Install this plugin
/plugin install threejs-kit@cypherpoet-toolchest
```

## Skills

| Skill | Description | Model-Invocable |
|---|---|---|
| [threejs-mastery](skills/threejs-mastery/SKILL.md) | Comprehensive Three.js reference covering WebGPU/WebGL rendering, shaders, animation, loaders, and post-processing. | Yes |
| [webgl-mastery](skills/webgl-mastery/SKILL.md) | Raw WebGL2 + GLSL shader fundamentals beneath Three.js (vendored from [webgl-kit](https://github.com/CypherPoet/custom-agent-skills/tree/main/plugins/webgl-kit)). | Yes |
