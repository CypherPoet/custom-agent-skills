...
**Going below the reconciler?** For Three.js itself — materials theory, lighting/IBL, GLTF internals, TSL/GLSL shading technique, `WebGPURenderer` specifics — use the sibling **`threejs-mastery`** skill (the `threejs-kit` dependency). This skill stays at the React layer: the reconciler, hooks, events, drei, and the pmndrs ecosystem.
...
- [`threejs-mastery` skill](https://github.com/CypherPoet/custom-agent-skills/blob/main/plugins/threejs-kit/skills/threejs-mastery/SKILL.md) — sibling skill for Three.js itself beneath the reconciler (ships with this plugin).
