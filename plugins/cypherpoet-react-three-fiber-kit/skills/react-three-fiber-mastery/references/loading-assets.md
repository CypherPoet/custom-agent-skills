...
`aoMap` and `lightMap` sample the **second** UV set — the attribute named `uv1` since three r151 (older tutorials say `uv2`, the pre-r151 name). GLTF exports with baked AO usually include `TEXCOORD_1` already; procedural geometry needs it copied:

```jsx
import { useEffect, useRef } from 'react'

function BakedPlane({ aoMap }) {
  const meshRef = useRef(null)
  useEffect(() => {
    const geometry = meshRef.current.geometry
    geometry.setAttribute('uv1', geometry.attributes.uv) // 'uv2' before three r151
  }, [])
  return (
    <mesh ref={meshRef}>
...
| `aoMap`/`lightMap` has no visible effect | They sample the second UV set: `geometry.setAttribute('uv1', geometry.attributes.uv)` (three r151+; `uv2` is the pre-r151 name). |
