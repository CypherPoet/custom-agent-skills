...
- **React.** Use [react-three-fiber](https://github.com/pmndrs/react-three-fiber) + [drei](https://github.com/pmndrs/drei) rather than mounting Three.js directly. The fundamentals in these references apply unchanged; for r3f-specific APIs (`useFrame`, `<Canvas>`, drei helpers), use the sibling **`react-three-fiber-mastery`** skill (in the `cypherpoet-react-three-fiber-kit` plugin).
