...
The reason to ship a `.icon` over a flat PNG is that the system re-renders your **layers** per appearance — Default, Dark, Tinted (monochrome), and Clear. Per-appearance values live in `icon.json` as a `<property>-specializations` array on a layer: the layer carries its default value (no `appearance` key), and each override names one appearance. So a two-edition icon can swap backgrounds by appearance via opacity:

```json
"layers": [
  { "image-name": "emblem.png", "name": "emblem" },
  { "image-name": "bg-parchment.png", "name": "bg-light",
    "opacity-specializations": [ { "appearance": "dark", "value": 0.0 } ] },
  { "image-name": "bg-ink.png", "name": "bg-dark",
    "opacity-specializations": [ { "appearance": "dark", "value": 1.0 } ] }
]
```

Author this in Icon Composer to get the exact key names right; the shape above is what to expect when you diff or script it.
