...
| Source repo (`origin`) | Marketplace repo (clone target) | Marketplace name (`@name`) | Codex display name |
|---|---|---|---|
| `CypherPoet/custom-agent-skills` | `CypherPoet/cypherpoet-toolchest` | `cypherpoet-toolchest` | `CypherPoet Toolchest` |
| `CypherPoet/private-custom-agent-skills` | `CypherPoet/cypherpoet-toolchest-private` | `cypherpoet-toolchest-private` | `CypherPoet Toolchest Private` |

**Adding a repo:** append a row. No skill edits needed — the lookup is data-driven.

**No matching row:** the skill asks which marketplace to target and which user-facing Codex display name to use rather than guessing. The marketplace name usually equals the marketplace repo's short name, but the `name` and `interface.displayName` fields in the Codex `marketplace.json` are authoritative.
