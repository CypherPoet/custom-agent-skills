...
export type ClaudeValidatorRunner = (
  root: string,
  pluginPath: string,
) => CommandResult;

const defaultRunner: ClaudeValidatorRunner = (root, pluginPath) => {
  const result = spawnSync(
    process.platform === "win32" ? "claude.cmd" : "claude",
    ["plugin", "validate", "--strict", pluginPath],
    {
      cwd: root,
      stdio: "inherit",
    },
...
export function authoredClaudePluginPaths(root: string): string[] {
...
export function validateAuthoredClaudePlugins(
  root: string,
  runner: ClaudeValidatorRunner = defaultRunner,
): string[] {
  ...
  for (const pluginPath of pluginPaths) {
    ...
    if (result.error !== undefined) {
      problems.push(
        `[claude-validator] could not run Claude Code for ${pluginPath}: ${result.error.message}. ` +
          "Run npm ci to install the repository's pinned validator.",
      );
    } else if (result.status !== 0) {
      problems.push(
        `[claude-validator] strict validation failed for ${pluginPath} ` +
