...
function pythonCommand(): { command: string; prefix: string[] } | undefined {
...
function parseArguments(arguments_: readonly string[]): { buildCheck?: string; valid: boolean } {
  let buildCheck: string | undefined;
...
function main(arguments_: readonly string[]): number {
  ...
  const pythonTests = existsSync(pythonTestsDirectory)
    ? readdirSync(pythonTestsDirectory).filter(
