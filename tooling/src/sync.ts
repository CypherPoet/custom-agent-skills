...
import { isJsonObject, type VendoredSkillsConfiguration } from "./types.js";
import {
...
function loadConfiguration(root: string): {
  ...
  return {
    configuration: value as unknown as VendoredSkillsConfiguration,
    problems: [],
  };
}

export function synchronizeVendoredSkills(rootPath: string, write: boolean): string[] {
...
export function findRepositoryRoot(start: string): string {
  ...
  for (;;) {
    ...
    if (parent === candidate) {
      throw new Error(
        `could not locate repo root (needs plugins/ and ${VENDORED_SKILLS_CONFIGURATION})`,
      );
    }
    candidate = parent;
  }
