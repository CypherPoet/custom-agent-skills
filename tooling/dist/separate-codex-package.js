...
export function stripClaudeInvocationField(skillManifest, label) {
    let text;
    try {
    ...
    const matchingPairs = document.contents.items.filter((pair) => isScalar(pair.key) && pair.key.value === invocationFieldName);
    ...
    if (keyLineStart !== valueLineStart ||
...
export function codexManualOnlyPolicyProblem(agentManifest) {
    let text;
    try {
...
export function codexPackageRelativePath(name, pluginMetadata) {
...
export function prepareSeparateCodexPackage(root, name, pluginDirectory, manifestBytes, visible) {
    ...
    if (manualOnlySkills.length === 0 && problems.length === 0) {
        problems.push(`[codex-package] ${name}: separateCodexPackage requires at least one ` +
...
//# sourceMappingURL=separate-codex-package.js.map