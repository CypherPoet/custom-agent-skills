import { CAPABILITY_MAX_COUNT, CAPABILITY_MAX_LENGTH, DEFAULT_PROMPT_MAX_COUNT, DEFAULT_PROMPT_MAX_LENGTH, DEVELOPER_NAME_MAX_LENGTH, DISPLAY_NAME_MAX_LENGTH, LONG_DESCRIPTION_MAX_LENGTH, SHORT_DESCRIPTION_MAX_LENGTH, SOURCE_HOMEPAGE_MAX_LENGTH, SUPPORTED_CODEX_CATEGORIES, WEBSITE_URL_MAX_LENGTH, } from "./constants.js";
...
export function normalizedUniquenessKey(value, ignoreCase) {
    const normalized = value.normalize("NFKC").trim().split(/\s+/u).join(" ");
    return ignoreCase ? normalized.toLocaleLowerCase("und") : normalized;
}
export function validateSubmissionText(value, field, maximumLength, allowLineFeed = false) {
    ...
    for (const character of value) {
        if ((controlCharacter.test(character) && !(allowLineFeed && character === "\n")) ||
...
function validateSubmissionUrl(value, field, maximumLength) {
...
function validateSubmissionStringList(value, field, options) {
...
function validateOptionalSubmissionStringList(value, field, options) {
    return value === undefined ? [] : validateSubmissionStringList(value, field, options);
}
function validateOptionalDefaultPrompt(value) {
    if (value === undefined) {
        return [];
    }
...
export function validateCodexSubmissionInterface(interfaceValue) {
    if (!isJsonObject(interfaceValue)) {
        return ["interface must be an object"];
    }
    const pluginInterface = interfaceValue;
    const problems = [];
    problems.push(...validateSubmissionText(pluginInterface.displayName, "interface.displayName", DISPLAY_NAME_MAX_LENGTH), ...validateSubmissionText(pluginInterface.shortDescription, "interface.shortDescription", SHORT_DESCRIPTION_MAX_LENGTH), ...validateSubmissionText(pluginInterface.longDescription, "interface.longDescription", LONG_DESCRIPTION_MAX_LENGTH, true), ...validateSubmissionText(pluginInterface.developerName, "interface.developerName", DEVELOPER_NAME_MAX_LENGTH));
    const maximumCategoryLength = Math.max(...Array.from(SUPPORTED_CODEX_CATEGORIES, (category) => category.length));
    problems.push(...validateSubmissionText(pluginInterface.category, "interface.category", maximumCategoryLength));
    if (typeof pluginInterface.category === "string" &&
        !SUPPORTED_CODEX_CATEGORIES.has(pluginInterface.category)) {
        problems.push(`interface.category must be one of: ${Array.from(SUPPORTED_CODEX_CATEGORIES).sort().join(", ")}`);
    }
    problems.push(...validateOptionalSubmissionStringList(pluginInterface.capabilities, "interface.capabilities", {
    ...
    if (pluginInterface.websiteURL !== undefined) {
        problems.push(...validateSubmissionUrl(pluginInterface.websiteURL, "interface.websiteURL", WEBSITE_URL_MAX_LENGTH));
    }
    return problems;
}
/** CypherPoet authoring choices layered on top of Codex's submission contract. */
export function validateRepositoryInterfacePolicy(interfaceValue, sourceHomepage) {
    if (!isJsonObject(interfaceValue)) {
        return [];
    }
    const pluginInterface = interfaceValue;
    const problems = [
        ...validateRequiredRepositoryList(pluginInterface.capabilities, "interface.capabilities"),
        ...validateRequiredRepositoryList(pluginInterface.defaultPrompt, "interface.defaultPrompt"),
...
export function validateGeneratedCodexInterface(interfaceValue, sourceHomepage) {
    return [
        ...validateCodexSubmissionInterface(interfaceValue),
        ...validateRepositoryInterfacePolicy(interfaceValue, sourceHomepage),
    ];
}
export function validateAuthoredRegistryInterface(name, pluginMetadata) {
    ...
    const maximumCategoryLength = Math.max(...Array.from(SUPPORTED_CODEX_CATEGORIES, (category) => category.length));
    problems.push(...validateSubmissionText(authoredInterface.displayName, "interface.displayName", DISPLAY_NAME_MAX_LENGTH), ...validateSubmissionText(authoredInterface.shortDescription, "interface.shortDescription", SHORT_DESCRIPTION_MAX_LENGTH), ...validateSubmissionText(pluginMetadata.category, "interface.category", maximumCategoryLength));
    if (typeof pluginMetadata.category === "string" &&
        !SUPPORTED_CODEX_CATEGORIES.has(pluginMetadata.category)) {
        problems.push(`interface.category must be one of: ${Array.from(SUPPORTED_CODEX_CATEGORIES).sort().join(", ")}`);
    }
    problems.push(...validateSubmissionStringList(authoredInterface.capabilities, "interface.capabilities", {
        maximumCount: CAPABILITY_MAX_COUNT,
        maximumItemLength: CAPABILITY_MAX_LENGTH,
    ...
    problems.push(...duplicateListProblems(authoredInterface.capabilities, "interface.capabilities", true), ...duplicateListProblems(authoredInterface.defaultPrompt, "interface.defaultPrompt", false));
    return problems.map((problem) => `${prefix}${problem}`);
}
//# sourceMappingURL=codex-submission-preflight.js.map