...
export function shippedPluginForPath(relativePath) {
...
export function runVersionBumpCheck(rootPath, baseArgument = "main", output = defaultOutput) {
    ...
    if (mergeBaseResult.status !== 0 || mergeBase.length === 0) {
        output.stdout(`check-version-bumps — SKIPPED: no merge base for ${base}...HEAD ` +
    ...
    output.stdout(`check-version-bumps — ${problems.length} ${problems.length === 1 ? "plugin" : "plugins"} ` +
