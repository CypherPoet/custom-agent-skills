...
export function directoryIgnored(name) {
    return IGNORED_DIRECTORY_NAMES.has(name) || name.endsWith("-workspace");
}
export function fileIgnored(name) {
...
export function gitText(root, arguments_) {
...
export function repositoryVisibleFiles(root) {
...
function walkTree(base, current, visible, files) {
...
export function readTree(base, visible) {
    const files = new Map();
...
export function writeTree(files, destination) {
    if (pathExistsOrIsSymbolicLink(destination)) {
        rmSync(destination, { recursive: true, force: true });
    }
    for (const [relativePath, data] of Array.from(files.entries()).sort(([left], [right]) => left.localeCompare(right, "en"))) {
        const target = resolve(destination, relativePath);
        mkdirSync(dirname(target), { recursive: true });
        writeFileSync(target, data);
    }
}
export function fileTreesEqual(left, right) {
...
export function validSkillPath(value) {
    ...
    return ((parts.length === 4 && parts[0] === "plugins" && parts[2] === "skills") ||
        (parts.length === 3 &&
...
export function desiredVendorTargets(configuration) {
    ...
    configuration.skills.forEach((edge, index) => {
        ...
        for (const target of targets) {
            ...
            if (previousSource !== undefined) {
                problems.push(`[vendor] duplicate target ${target}: declared by ${previousSource} and ${source}`);
                continue;
            }
    ...
    for (const source of Array.from(sources).sort()) {
        if (desired.has(source)) {
            problems.push(`[vendor] vendoring chains are not allowed: source is also a target: ${source}`);
        }
    }
    return { desired, problems };
}
function vendorTargetsAt(root, reference) {
...
function vendoringComparisonBase(root) {
...
export function skillDirectories(root) {
    ...
    if (existsSync(pluginsDirectory)) {
        for (const plugin of readdirSync(pluginsDirectory, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name, "en"))) {
            ...
            for (const skill of readdirSync(skillsDirectory, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name, "en"))) {
...
export function baseVisibleFiles(visible, baseRelative) {
...
export function pathIsSymbolicLink(path) {
    try {
...
export function pathExistsOrIsSymbolicLink(path) {
    try {
        lstatSync(path);
        return true;
    }
