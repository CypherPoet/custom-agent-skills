...
import { CODEX_MANIFEST_CARRY, CODEX_PACKAGES_DIRECTORY, PLUGIN_DESCRIPTION_MAX_LENGTH, REGISTRY, SYNC_COMMAND, } from "./constants.js";
import { baseVisibleFiles, desiredVendorTargets, fileTreesEqual, gitCleanUnder, pathExistsOrIsSymbolicLink, pathIsSymbolicLink, previousVendorTargets, readTree, repositoryVisibleFiles, skillDirectories, treeDigest, writeTree, } from "./file-tree.js";
import { normalizedUniquenessKey, validateAuthoredInterfaceMetadata, validateCodexInterface, validateText, } from "./interface-contract.js";
...
export function codexPluginRelativePath(name, pluginMetadata) {
    return isJsonObject(pluginMetadata) && pluginMetadata.codexProjection === true
        ? `${CODEX_PACKAGES_DIRECTORY}/${name}`
        : `plugins/${name}`;
}
function validateClaudeManifestShape(name, claudeManifest) {
    ...
    problems.push(...validateText(claudeManifest.description, "Claude manifest description", PLUGIN_DESCRIPTION_MAX_LENGTH, true).map((problem) => `${prefix}${problem}`));
...
function unportedComponents(pluginDirectory, claudeManifest) {
    const unported = ["mcpServers", "hooks", "agents", "commands"].filter((key) => Object.hasOwn(claudeManifest, key));
...
function stripClaudeInvocationField(skillManifest, label) {
    let text;
    try {
    ...
    const frontmatterEnd = lines.findIndex((line, index) => index > 0 && line.replace(/\r?\n$/u, "") === "---");
    if (frontmatterEnd < 0) {
        return {
            manualOnly: false,
    ...
    for (let index = 1; index < frontmatterEnd; index += 1) {
        ...
        if (match !== null) {
            ...
            problems.push(`[codex-package] ${label}: Claude invocation field must use a YAML boolean`);
        }
    }
    if (matches.length > 1) {
...
function codexManualOnlyPolicyProblem(agentManifest) {
    let text;
    try {
...
function prepareCodexPackage(root, name, pluginDirectory, manifestBytes, visible) {
    ...
    const problems = [];
    for (const [path, data] of Array.from(packageTree.entries()).sort(([left], [right]) => left.localeCompare(right, "en"))) {
    ...
    if (manualOnlySkills.length === 0 && problems.length === 0) {
        problems.push(`[codex-package] ${name}: codexPackage requires at least one Claude-only disable-model-invocation field`);
    }
    for (const skillName of manualOnlySkills) {
        ...
        const policyProblem = agentManifest === undefined
            ? "agents/openai.yaml is missing"
            : codexManualOnlyPolicyProblem(agentManifest);
        if (policyProblem !== undefined) {
            problems.push(`[codex-package] ${name}/${skillName}: removing Claude's manual-only field requires ` +
...
function prepareCodexPlan(root, dualPlugins, visible) {
    ...
    for (const [name, pluginMetadata] of Object.entries(dualPlugins).sort(([left], [right]) => left.localeCompare(right, "en"))) {
        ...
        if (previousOwner !== undefined) {
            problems.push(`[config] ${name}: interface.displayName duplicates ${previousOwner} after normalization: ${JSON.stringify(displayName)}`);
        }
        else {
    ...
    for (const [name, pluginMetadata] of Object.entries(dualPlugins).sort(([left], [right]) => left.localeCompare(right, "en"))) {
        ...
        const manifestRead = readJsonObject(join(pluginDirectory, ".claude-plugin", "plugin.json"), `[codex-manifest] ${name}`);
        ...
        if (unported.length > 0) {
            problems.push(`[codex-manifest] ${name}: Claude-only components (${unported.join(", ")}) — ` +
        ...
        problems.push(...interfaceProblems.map((problem) => `[codex-manifest] ${name}: ${problem}`));
        ...
        if (pluginMetadata.codexProjection === true) {
            const prepared = prepareCodexPackage(root, name, pluginDirectory, manifestBytes, visible);
...
function synchronizeVendoredSkills(root, configuration, write, visible) {
...
function applyCodexPlan(root, plan, separatePackagePlugins, claudeOnlyPlugins, existingPlugins, write, visible) {
    const problems = [];
    for (const [path, desiredBytes] of Array.from(plan.manifests.entries()).sort(([left], [right]) => left.localeCompare(right, "en"))) {
        if (write) {
            mkdirSync(dirname(path), { recursive: true });
            writeFileSync(path, desiredBytes);
        }
        else if (!existsSync(path) || !readFileSync(path).equals(desiredBytes)) {
            problems.push(`[codex-manifest] out of sync: ${relative(root, path)} (run: ${SYNC_COMMAND})`);
        }
    }
    for (const [path, desiredTree] of Array.from(plan.packages.entries()).sort(([left], [right]) => left.localeCompare(right, "en"))) {
    ...
    for (const name of Array.from(separatePackagePlugins).filter((value) => existingPlugins.has(value)).sort()) {
        ...
        if (!write) {
            problems.push(`[codex-package] stale in-place Codex manifest: ${relativePath} (run: ${SYNC_COMMAND})`);
        }
    ...
    const existingPackageNames = existsSync(packagesRoot)
        ? new Set(readdirSync(packagesRoot, { withFileTypes: true })
    ...
    for (const name of Array.from(existingPackageNames).filter((value) => !separatePackagePlugins.has(value)).sort()) {
        ...
        else if (pathIsSymbolicLink(stale) ||
    ...
    for (const name of Array.from(claudeOnlyPlugins).filter((value) => existingPlugins.has(value)).sort()) {
        ...
        if (write) {
            ...
            problems.push(`[codex-manifest] stale .codex-plugin/ for Claude-only plugin ${name} (run: ${SYNC_COMMAND})`);
        }
    }
    return problems;
}
export function synchronizePlugins(rootPath, write) {
    ...
    const existingPlugins = new Set(readdirSync(pluginsDirectory, { withFileTypes: true })
    ...
    for (const name of Array.from(existingPlugins).sort()) {
        if (!dualPluginNames.has(name) && !claudeOnlyPlugins.has(name)) {
            problems.push(`[config] plugins/${name}/ is unclassified — add it to dual_harness_plugins or claude_only_plugins`);
        }
    }
