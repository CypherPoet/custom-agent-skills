...
export function validateText(value, field, maximumLength, allowLineFeed = false) {
...
export function validateUrl(value, field, maximumLength) {
...
function validateStringList(value, field, options) {
    if (!Array.isArray(value)) {
        return [`${field} must be an array`];
    }
    const problems = [];
    if (value.length < options.minimumCount || value.length > options.maximumCount) {
        problems.push(`${field} must contain between ${options.minimumCount} and ${options.maximumCount} entries`);
    }
...
export function validateCodexInterface(interfaceValue, options = {}) {
    if (!isJsonObject(interfaceValue)) {
        return ["interface must be an object"];
    }
    const pluginInterface = interfaceValue;
    const problems = [];
    problems.push(...validateText(pluginInterface.displayName, "interface.displayName", DISPLAY_NAME_MAX_LENGTH), ...validateText(pluginInterface.shortDescription, "interface.shortDescription", SHORT_DESCRIPTION_MAX_LENGTH), ...validateText(pluginInterface.longDescription, "interface.longDescription", LONG_DESCRIPTION_MAX_LENGTH, true), ...validateText(pluginInterface.developerName, "interface.developerName", DEVELOPER_NAME_MAX_LENGTH));
    const maximumCategoryLength = Math.max(...Array.from(SUPPORTED_CODEX_CATEGORIES, (category) => category.length));
    problems.push(...validateText(pluginInterface.category, "interface.category", maximumCategoryLength));
    ...
    problems.push(...validateStringList(pluginInterface.capabilities, "interface.capabilities", {
        minimumCount: 1,
    ...
    if (options.sourceHomepage !== undefined) {
        problems.push(...validateUrl(options.sourceHomepage, "source homepage", SOURCE_HOMEPAGE_MAX_LENGTH));
...
export function validateAuthoredInterfaceMetadata(name, pluginMetadata) {
