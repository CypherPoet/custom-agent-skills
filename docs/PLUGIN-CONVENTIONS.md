...
The same rule covers the per-plugin `README.md`: it ships in the sparse-clone install alongside the skills, so a cross-plugin relative link there is just as dead in an installed copy — use an absolute GitHub URL there too. (`check-skill-structure.py` currently enforces skill files only; the weekly health audit catches README links.)
