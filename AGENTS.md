...
Public collection of reusable AI agent skills, packaged as Codex plugins that can be distributed through [plugin marketplaces](https://code.Codex.com/docs/en/plugin-marketplaces).

## Architecture

- `plugins/<plugin-name>/` — Each themed Codex plugin. Self-contained — a `git-subdir` sparse-clone fetches only this directory when a consumer installs the plugin. Plugin shape follows the [Codex plugins reference](https://code.Codex.com/docs/en/plugins-reference); this repo's deltas live in [`docs/PLUGIN-CONVENTIONS.md`](docs/PLUGIN-CONVENTIONS.md).
...
- Maintainer skills (marketplace + catalog tooling) — provided by the **`cypherpoet-marketplace-kit`** plugin, enabled in `.Codex/settings.json` (not repo-local; see [Maintainer Skills](#maintainer-skills)).
...
**Updating a plugin:** bump its `version` in `plugin.json` whenever a content change should reach already-installed users — pushing to `main` alone won't (`version` is Codex's update cache key).

## Maintainer Skills

These come from the **`cypherpoet-marketplace-kit`** plugin (enabled in `.Codex/settings.json`), not repo-local copies — this repo and the private sibling install the same published source. All run locally (no tokens, no CI):

- **`marketplace-publish`** — publish one plugin to `cypherpoet-toolchest` by opening a PR. Manual-only (`disable-model-invocation`); it has an outward-facing side effect, so Codex shouldn't auto-run it.
