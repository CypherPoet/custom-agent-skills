...
**Scope — local catalog only.** This rewrites `docs/CATALOG.md` and nothing else. It does **not** touch the published marketplace (`marketplace.json` in `cypherpoet-toolchest`), commit, or open a PR. The marketplace is a deliberate, per-plugin publish — for that, use `marketplace-publish`.
