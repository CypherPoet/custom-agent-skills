...
- The **local catalog** should mirror `plugins/` *exactly* — the refresh rule is "new plugin → add a row" ([PLUGIN-CONVENTIONS.md](../../../docs/PLUGIN-CONVENTIONS.md)), so a missing or stale row *is* an oversight. Fixes are a plain edit to `docs/CATALOG.md` (a normal docs commit/PR), never a publish.
...
- **`docs/CATALOG.md`** missing / stale / orphan rows → a plain edit to that file (a normal docs commit/PR). **Not** `marketplace-publish` — the local catalog isn't the marketplace.
